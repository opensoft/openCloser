...
def test_constitution_lists_five_principles() -> None:
    text = _CONSTITUTION.read_text(encoding="utf-8")
    for heading in (
        ...
        assert heading in text, f"Constitution is missing principle heading: {heading!r}"


def test_constitution_traceability_section_references_known_FRs() -> None:
    """Each principle traces to specific FRs/SCs that exist in the current spec."""
    text = _CONSTITUTION.read_text(encoding="utf-8")
    # The Traceability section should reference at least these FRs (sampled).
    for fr_or_sc in ("FR-008", "FR-016", "FR-033", "FR-010", "FR-018", "FR-035", "FR-019", "FR-020", "FR-021", "SC-005", "SC-006", "SC-009"):
        assert fr_or_sc in text, f"Constitution Traceability section should mention {fr_or_sc}"
