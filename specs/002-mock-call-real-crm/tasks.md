...
- [ ] T014 Create `src/opencloser/crm/dataverse/queue_loader.py` — `DataverseQueueLoader.load(selector)` mapping a Dataverse row → the unchanged `QueueItem` contract; deterministic next-ready ordering; empty queue → `None` (FR-008, contracts/dataverse-queue-loader.md)
...
- **US6** depends only on Phase 2 — touches `redaction/` and `artifacts/writer.py`; no Dataverse code. Can run in parallel with US1–US4 (coordinate the `artifacts/writer.py` edit with T026).
