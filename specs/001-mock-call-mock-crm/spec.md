...
- **FR-004**: System MUST evaluate eligibility for the target queue record using at least these rules, in this canonical order: (a) phone presence, (b) usable timezone (record-supplied or configured default), (c) current local time within the configured call window, (d) DNC / opt-out flag not set, (e) attempt count below the configured maximum, and (f) callable-status equals `ready` (any other value — `in_progress`, `completed`, `blocked`, `dnc` — MUST block the call). The system MUST evaluate ALL six rules on every record (no short-circuit) and persist each rule's pass/fail result. When multiple rules fail, the persisted decision MUST list every failing rule by name, preserving rule order (a)–(f). The set of rule results MUST be persisted with the queue record's eligibility decision.
...
- **FR-010**: The persona MUST disclose, in its first utterance after the `connected` event, that it is an AI assistant calling on behalf of Medx. The persona MUST remain non-clinical, MUST NOT collect resident or patient health information, and MUST honor DNC / opt-out statements by terminating the sales flow before any further sales utterance. The persona MUST mark uncertainty for human review with a stated reason drawn from FR-035's enumerated reason-code set.
...
- **FR-030**: The Task payload MUST include, at minimum: `task_kind` (one of `callback` / `review`), `subject`, `reason_code` (from FR-035's enumeration for review tasks; not applicable for callback tasks), `session_id`, `queue_item_id`, `persona_version`, `created_at`, and — when `task_kind=callback` — `preferred_callback_window` AND `captured_email` when the captured email is verified (per the Q5 clarification). It MUST NOT be emitted for dispositions excluded by FR-018.
...
- **FR-034**: The persona's extraction schema MUST include, at minimum, the following fields per connected conversation: `captured_email` (verified per the Slice 1 verified-email assumption), `captured_email_unverified` (when captured but unverified), `callback_requested` (boolean), `preferred_callback_window` (when `callback_requested`), `role_confidence` (one of `confident_decision_maker` / `confident_non_decision_maker` / `uncertain`), `intent_classification` (one of `interested` / `not_interested` / `call_back_later` / `dnc_stated` / `wrong_number` / `uncertain`), and `refusal_topics` (list of topic categories the contact declined to discuss, when applicable).
- **FR-035**: The persona MUST emit `needs_human_review` with a `human_review_reason` drawn from this enumerated set: `uncertain_role`, `uncertain_intent`, `ambiguous_dnc`, `captured_email_invalid_no_callback`, `phi_collection_risk`, `legal_request`, `non_clinical_topic_escalation`, `outside_allowed_claims`, `script_truncated`. Additional reason codes MAY be added in future slices but Slice 1 MUST NOT introduce free-form reasons.
